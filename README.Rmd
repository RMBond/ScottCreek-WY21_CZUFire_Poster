...
3. The <span style="color:purple">*Scott_Creek_Pebble_XXX.csv*</span> datafile consists of pebble counts at 23 transects accrost the watershed (6 eFishing sites with 3 transects each, and 5 additional pebble count only transects) which were repeated twice (Autumn 2020 & Winter 2021). AC and MA entered and QC'ed the data. 
...
3. RB is working on code for making pebble count summaries and potential plotting.
...
#Test Data
pc.dat.test <- read.csv("Data/Scott_Creek_Pebble_Testdata_20210315.csv", sep = ",", header = T) #Initial pebble count test dataset.
...
test.bc3 <- pc %>% 
  filter(Site == "Big Creek eFishing",
         Transect == 3) #32 obs of 8 var

#Plot
ggplot(test.bc3, aes(Size_class_mm, Percent_finer, color = Round)) +
  geom_line() +
...
Start by summarising the % fines at each transect and then subtract round 1 from round 2.
...
```{r Caculate change in % fines (<=8mm) for each transect}
#Subset data by Round
pc3.r1.8 <- pc %>%
  filter(Round == 1) %>% #Round 1 data
