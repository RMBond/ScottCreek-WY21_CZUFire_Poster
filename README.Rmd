...
#A quick plot of the test data
ggplot(pc, aes(Size_class_mm, Percent_finer, color = Round)) +
  geom_line() +
...
pc3.r2 <- pc %>% 
  filter(Size_class_mm == 5.6) %>% #focus on the 5.6 size class for each transect
  filter(Round = 1) #Round 2 data

#Spread data into long format to calculate % change
pc3 <- full_join(pc3.r1, pc3.r2) %>% #Joins the two rounds of data
